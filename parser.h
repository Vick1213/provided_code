...
private:
    ...
    int calc_Poly(std::vector<Token> &tokens, int & pos,const std::vector<variable>active);
    int cal_assign(AssignmentF* assin, std::vector<variable> &active );
    int cal_Poly(poly_decl_t p, const std::vector<int>& args);
    ...
    int recur_add_poly(std::vector<Token> addtokens);
public:
